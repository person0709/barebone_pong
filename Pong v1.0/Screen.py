...
class Screen(object):
    ...
    def draw_char(self, x, y, char, colour):
        for row in range(len(constants.LETTERS[char])):
            for i in range(len(constants.LETTERS[char][row])):
                if (constants.LETTERS[char][row][i] > 0):
                    self.draw_rect(x+i, y+row, (1, 1), colour)
                else:
                    self.draw_rect(x+i, y+row, (1, 1), constants.BACKGROUND_COLOUR)
